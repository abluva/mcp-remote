...
/**
 * Regression tests for issue #306 (missing `Mcp-Method` / `Mcp-Name` on the legacy
 * Streamable HTTP path), ported from upstream PR #309 and adapted to the Abluva fork.
 *
 * Fork adaptation: connectToRemoteServer runs protocol auto-detection first, which would
 * fire its own server/discover probe against the recording server. These tests pin
 * protocolMode to 'legacy' (the exact #306 scenario) so the only requests the server sees
 * are the ones the transport itself sends.
 */
...
describe('Feature: Method-aware MCP HTTP gateways (issue #306 / PR #309)', () => {
  ...
  it('Scenario: resources/read and prompts/get source Mcp-Name from uri/name', async () => {
    // Given a server that records the headers it receives
    const gateway = createHeaderRecordingServer()
    const url = await gateway.start()
...
describe('Feature: Encoding MCP header values (PR #309)', () => {
